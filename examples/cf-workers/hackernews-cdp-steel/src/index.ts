/**
 * Hacker News scraper - CDP with Steel.dev provider on Cloudflare Workers.
 *
 * This example demonstrates how to use CDP (lightweight, no dependencies)
 * with Steel.dev provider for browser automation.
 *
 * ## Steel SDK Features
 *
...
const scrapeHackerNews = Effect.gen(function* () {
  ...
  const stories = yield* cdp.withSession({ provider }, ({ page, session }) =>
    Effect.gen(function* () {
      ...
      // ───────────────────────────────────────────────────────────────────
      // Scraping: Extract stories from the page (runs in browser context)
      // ───────────────────────────────────────────────────────────────────
...
<body>
	...
	<p>Cloudflare Workers example using CDP + Steel.dev</p>
