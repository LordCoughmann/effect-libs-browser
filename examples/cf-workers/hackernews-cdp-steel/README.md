# Hacker News CDP Scraper - Steel.dev

A Hacker News scraper running on Cloudflare Workers using CDP with Steel.dev provider.
...
You'll see the JSON results in the browser. Check the wrangler logs for progress updates (connecting to Steel, session created, etc.).

## See Also

- `hackernews-cdp-browserbase/` - Browserbase provider
- `hackernews-cdp-wsUrl/` - Custom CDP URL (no SDK)
- `hackernews-playwright-steel/` - Same example using Playwright instead of raw CDP
...
The scraper uses CDP (Chrome DevTools Protocol) with Steel.dev as the browser provider to:
