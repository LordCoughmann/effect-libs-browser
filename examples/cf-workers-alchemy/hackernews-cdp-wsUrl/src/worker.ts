/**
 * CDP Worker — runs inside Cloudflare Workers via Alchemy.
 *
...
const scrapeHackerNews = (cdpUrl: string) =>
  Effect.gen(function* () {
    ...
    return yield* cdp.withConnection({ url: cdpUrl }, ({ page }) =>
      Effect.gen(function* () {
...
<body>
	...
	<p>Cloudflare Workers + Alchemy (Bun) example using CDP with custom URL</p>
...
export default class HnCdpWorker extends Cloudflare.Worker<HnCdpWorker>()(
  "HnCdpWorker",
  {
    // import.meta.filename resolves to this file's absolute path at runtime
